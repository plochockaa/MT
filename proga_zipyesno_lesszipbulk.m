...
function [togrowsegm, thetasegm] = proga_zipyesno_lesszipbulk(theta1,theta2, thetacr,Pcat)
   ...
   if th3 < thetacr,            % if theta < thetacr
       if rn < th3*Pcat/thetacr,    
           togrowsegm = 0; 
           thetasegm  = 0; % catastrophe
       elseif (th3*(Pcat-1)/thetacr)+1 <= rn % <edit!>
           togrowsegm = 2;  thetasegm = theta1; % CROSS!! <edit>
       else
           togrowsegm = 1;                % zip
