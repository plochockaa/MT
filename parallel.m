...
thetacrmt   = 30 *pi/180;  % >>> the angle for zipping -- if set (thetacrmt = 0), then will have straight MTs (the code runs fater then, can reduce the number of segments to 1 or 2)
thetacrwall = 30 *pi/180;   % >>> the angle for zipping along the wall
...
Nactin_array = 80;
type_act = 2; % Type of actin in the cell, 1 - random, 2 - parallel
...
%%%%%%%%%%%%%%%%% ap bp  a    b   pcat
parameter_array = [4 1 1000 3500 0.01];

pcat_array = [0.01 0.05 0.1 0.15 0.2];

 for ff = 1:40,   % can run a gazillion of different cases and save the output in different directories
  ...
  for kk =1 :length(ecc_array), 
     ...
     for mm = 1:length(pcat_array);
    
         pcat_actin = pcat_array(mm);
    %%%%%%%%%%%%%%%%%%%%%% Directories in ssh space
      
      
   dir_name = ['/home/s1478806/RUNpar/RESULTS/'];
    ...
    theta_actin = (25.2*pi/180)*(ecc==0.7) + (19.8*pi/180)*(ecc==0.75) + ...
     ...
     Pcat = parameter_array(1,5);

    
    %______( Markov chain transition rates )_____________

    alphapr  = parameter_array(1,1); 
    betapr   = parameter_array(1,2);
    alpha    = parameter_array(1,3);
    beta     = parameter_array(1,4);
