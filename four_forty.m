...
thetacrmt   = 40 *pi/180;  % >>> the angle for zipping -- if set (thetacrmt = 0), then will have straight MTs (the code runs fater then, can reduce the number of segments to 1 or 2)
thetacrwall = 40 *pi/180;   % >>> the angle for zipping along the wall
...
parameter_array = [4 1 1000 3500 0.01];
 ...
 for ff = 1:20,   % can run a gazillion of different cases and save the output in different directories
  ...
  for kk =1 :length(ecc_array), 
    ...
    for mm = 1:length(parameter_array(:,1));
        
    %%%%%%%%%%%%%%%%%%%%%% Directories in ssh space
      
      
   dir_name = ['/home/ap45/Documents/RUNpar/RESULTS/'];
    ...
    Pcat = parameter_array(mm,5);


    
    %______( Markov chain transition rates )_____________

    alphapr  = parameter_array(mm,1); 
    betapr   = parameter_array(mm,2);
    alpha    = parameter_array(mm,3);
    beta     = parameter_array(mm,4);
